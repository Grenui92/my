...
import os
from contact_book.contact_book.classes.class_record import Record, Birthday
...
class AddressBook(UserDict):
    ...
    def show_upcoming_birthdays(self, n=7) -> list:
        ...
        for k, v in self.data.items():
            if isinstance(v.birthday, Birthday):
                ...
                # Проверяем входит ли дата следующего дня рождения в ближайшие Н дней
    ...
    def clear_contact(self, name: str, field: str) -> str:
        record = self.data[name]
        return record.clear_contact(field=field)
