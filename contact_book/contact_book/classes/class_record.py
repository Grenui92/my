...
class Record:
    ...
    def clear_contact(self, field):
        if field:
            self.__dict__[field] = []
            return f"Field {field} successfully cleared for contact {self.name.value}."
        else:
            for f in self.__dict__.keys():
                if f != "name":
                    self.__dict__[f] = []
            return f"Contact {self.name.value} successfully cleared."
...
class Phone(Field):
    ...
    @Field.value.setter
    def value(self, value):
        data = re.findall(r"[+]\d{12}|[+]38.\d{3}.\d{3}-\d{2}-\d{2}|[+]38-\d{5}-\d{2}-\d{2}-\d", value)
        if data:
